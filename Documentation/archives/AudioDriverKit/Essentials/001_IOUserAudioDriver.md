# 001_IOUserAudioDriver.pdf

## Page 1

For the Core Audio host to match against this driver, add the following keys to IOKit
Personalities, in the driver’s Info.plist file:
After matching the host with the driver, the AudioDriverKit framework creates the connection to th
Core Audio HAL as soon as the IOService calls NewUserClient. The driver extension must
have the com.apple.developer.driverkit.allow-any-userclient-access
entitlement.
Overview
Topics
...
A four character code which, along with the scope and element, specific piece of information
about an audio object.
AddCustomProperty
