...
A component expression that makes up part of a predicate, and that’s supported by the
standard predicate type.
...
API Collection
Filters and Sorting
Use predicates, expressions, and sort descriptors to examine elements in
...
class NSPredicate
A definition of logical conditions for constraining a search for a fetch or for in-memory
filtering.
class NSExpression
...
An immutable description of how to order a collection of objects according to a property
...
A comparator that compares types according to their conformance to the comparable
protocol.
struct KeyPathComparator
A comparator that uses another sort comparator to provide the comparison of values at a ke
path.
enum SortOrder
