...
class CSVFormatDebugger {
    ...
    static func testParsing(csvPath: String) {
        ...
        for (index, line) in lines.dropFirst().prefix(3).enumerated() {
            ...
            let fields =
                CSVParser.parseTransportLinesCSV(csvContent: line).first.map {
                    [
                        $0.routeId, $0.routeShortName, $0.routeLongName, $0.routeType,
                        $0.routeColor, "", $0.shape,
                    ]
                } ?? []
            ...
            if fields.count >= 7 {
                ...
                let cleanings = [
                    (
                        "Standard",
                        shapeRaw.replacingOccurrences(of: "\"\"", with: "\"").trimmingCharacters(
                            in: CharacterSet.whitespacesAndNewlines)
                    ),
                    (
                        "Sans guillemets externes",
                        shapeRaw.hasPrefix("\"") && shapeRaw.hasSuffix("\"")
                            ? String(shapeRaw.dropFirst().dropLast()).replacingOccurrences(
                                of: "\"\"", with: "\"") : shapeRaw
                    ),
                ]
                    ...
                    if let jsonData = cleaned.data(using: String.Encoding.utf8) {
                        do {
                            let geoJSON = try JSONDecoder().decode(
                                GeoJSONShape.self, from: jsonData)
                            print(
                                "    ✅ Parsing réussi! Type: \(geoJSON.type), Segments: \(geoJSON.coordinates.count)"
                            )
