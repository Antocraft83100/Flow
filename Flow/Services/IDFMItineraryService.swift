...
class IDFMItineraryService: ObservableObject {
    ...
    func searchItinerary(from: CLLocationCoordinate2D, to: Station, date: Date = Date(), isArrival: Bool = false) -> AnyPublisher<[Journey], Error> {
        ...
        return URLSession.shared.dataTaskPublisher(for: request)
            ...
            .handleEvents(receiveOutput: { data in
                ...
                if let str = String(data: data, encoding: .utf8) { print("📥 Itinerary Data: \(str)") }
            })
